...
int crear_conexion_cliente(char* ip, char* puerto) {
	char puerto_str[8];
	snprintf(puerto_str, sizeof(puerto_str), "%s", puerto);
	int err;
...
int crear_conexion_cliente_worker(char* ip, int puerto, t_log* logger) {
	char puerto_str[8];
	...
	if(socket_cliente == -1)
	{
		log_info(logger, "Error al crear el socket_cliente");
	}else{
		log_info(logger, "Creado correctamente el socket_cliente");
	}
	// Ahora que tenemos el socket, vamos a conectarlo
	...
	if(err == -1)
	{
		...
		log_info(logger, "Conectado correctamente!");
	}
